...
def connection():
    conn=pymysql.connect(
        host='localhost', user='root', password='1111', db='student_db'
    )
    return conn
...
def update():
    ...
    if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE students SET STUDID='" + studid + "',FNAME='" + fname + "',LNAME='" + lname + "',ADDRESS='" + address + "',PHONE='" + phone + "' WHERE STUDID='"+selectedStudid+"'")
            conn.commit()
            conn.close()
        except:
            messagebox.showinfo("Error", "Stud ID already exist")
...
sutdidLabel=Label(root, text="Stud ID", font=('Arial Bold', 15))
fnameLabel=Label(root, text="FirstName", font=('Arial Bold', 15))
lnameLabel=Label(root, text="LastName", font=('Arial Bold', 15))
addressLabel=Label(root, text="Address", font=('Arial Bold', 15))
phoneLabel=Label(root, text="Phone", font=('Arial Bold', 15))

sutdidLabel.grid(row=3, column=0, columnspan=1, padx=50, pady=5)
...
#textvariable
studidEntry=Entry(root, width=55, bd=5, font=('Arial Bold', 15), textvariable=ph1)
fnameEntry=Entry(root, width=55, bd=5, font=('Arial Bold', 15), textvariable=ph2)
lnameEntry=Entry(root, width=55, bd=5, font=('Arial Bold', 15), textvariable=ph3)
addressEntry=Entry(root, width=55, bd=5, font=('Arial Bold', 15), textvariable=ph4)
phoneEntry=Entry(root, width=55, bd=5, font=('Arial Bold', 15), textvariable=ph5)
...
selectBtn=Button(root, text="Select", padx=65, pady=25, width=10, bd=5, font=('Arial', 15), bg="#EEEEEE", command=select)
...
my_tree.column("FirstName", anchor=W, width=170)
my_tree.column("LastName", anchor=W, width=170)
